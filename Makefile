...
# 源文件
c_source_files := main.c auth.c sighandle.c rawsock.c
c_header_files := debug.h

# 输出文件
executable_files := cuit-client
...
.PHONY: install
install:
	cp $(executable_files) $(desdir)
	mkdir /usr/local/cuit-client/
#------------------------------------
# make uninstall
# 删除程序
.PHONY: uninstall
uninstall:
	$(RM) $(desdir)$(executable_files)
	rm -fr /usr/local/cuit-client/
#------------------------------------
# make clean
